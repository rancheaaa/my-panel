...
JAR_NAME_PATTERN="my-panel-admin*.jar"
...
if [ -d "$ROOT_DIR/libs" ]; then
    JAR_PATH=$(ls -t "$ROOT_DIR"/libs/$JAR_NAME_PATTERN 2>/dev/null | head -n 1)
fi

if [ -z "$JAR_PATH" ]; then
    if [ -f "$ROOT_DIR/$JAR_NAME_PATTERN" ]; then
        JAR_PATH=$(ls -t "$ROOT_DIR"/$JAR_NAME_PATTERN | head -n 1)
    fi
fi

if [ -z "$JAR_PATH" ]; then
    error "$JAR_NAME_PATTERN not found in root or libs directory."
    exit 1
fi
...
get_app_pid() {
    ...
    if [ -n "$jps_cmd" ]; then
        pid=$($jps_cmd -l | grep "my-panel-admin*" | awk '{print $1}' | head -n 1)
    fi
...
do_start_app() {
    ...
    mkdir -p "$LOG_DIR"
    cd "$ROOT_DIR"
    nohup $JAVA_CMD $JAVA_OPTS -jar "$JAR_PATH" > /dev/null 2>&1 &
    PID=$!
    cd "$BIN_DIR"
    echo $PID > "$PID_FILE"
...
if [ "$ACTION" = "app" ] || [ "$ACTION" = "nginx" ]; then
    TARGET=$ACTION
    ACTION=$2
fi
...
case "$ACTION" in
    ...
    *)
        ...
        echo "Usage: $0 [app|nginx] {install|start|stop|restart|status}"
        echo ""
        echo "Example:"
        echo "  $0 app start      # Only start the Java application"
        echo "  $0 nginx status   # Only check Nginx status"
        echo "  $0 start          # Start both application and Nginx"
        echo ""
        echo "Actions:"
