...
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  Level 1: 全局信号量 (Semaphore)                     │   │
│  │  • 控制整个Agent的最大并发上传数                       │   │
│  │  • 默认值: max_concurrent_uploads = 3                 │   │
│  │  • 范围: [1, 10]                                     │   │
│  └─────────────────────────────────────────────────────┘   │
│                           │                                 │
│                           ▼                                 │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  Level 2: 任务级并发控制器 (TaskConcurrencyLimiter)   │   │
│  │  • 每个任务可配置独立的并发数限制                        │   │
│  │  • 默认继承全局配置, 可覆盖为任务专属值                  │   │
│  │  • 示例: task_1001.max_concurrent = 5                  │   │
│  └─────────────────────────────────────────────────────┘   │
│                           │                                 │
│                           ▼                                 │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  Level 3: 上传线程池 (UploadThreadPool)               │   │
│  │  • 核心线程数 = min(全局上限, CPU核心数)                │   │
│  │  • 最大线程数 = 全局上限 * 2                            │   │
│  │  • 队列容量: 1000 (有界队列, 防止OOM)                   │   │
│  │  • 拒绝策略: CallerRunsPolicy (调用方执行, 限流效果)    │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
...
| 层级                 | 实现位置         | 类/组件                     | 配置来源                                                  |
| ------------------ | ------------ | ------------------------ | ----------------------------------------------------- |
| **Level 1: 全局信号量** | Source Agent | `BatchTransferManager`   | `agent.yml` → `batch.transfer.max-concurrent-uploads` |
| **Level 2: 任务级控制** | Source Agent | `TaskScheduler` (每个任务实例) | 任务配置字段 → `max_concurrent_uploads` (可选覆盖)              |
| **Level 3: 线程池**   | Source Agent | `UploadExecutorService`  | 根据Level 1动态计算                                         |
| **Proxy端限流**       | Proxy服务      | `ProgressRateLimiter`    | `proxy.yml` → `batch.progress.rate-limit.*`           |
...
public class BatchTransferManager {
    ...
    // 配置参数
    private int maxConcurrentUploads;      // 默认3, 范围[1,10]
    
    @PostConstruct
    public void init() {
        // 从配置文件或系统属性读取
        this.maxConcurrentUploads = config.getMaxConcurrentUploads();
        
        // 创建公平信号量(避免饥饿)
        this.globalUploadSemaphore = new Semaphore(maxConcurrentUploads, true);
        
        log.info("初始化批量传输管理器, 最大并发上传数: {}", maxConcurrentUploads);
    }
     ...
     * @return 许可证ID(用于释放)
     */
    public UploadPermit acquireUploadPermit() throws InterruptedException {
        globalUploadSemaphore.acquire();
        return new UploadPermit(globalUploadSemaphore);
    }
    
    /**
     * 尝试获取上传许可(非阻塞, 用于快速失败场景)
     */
    public Optional<UploadPermit> tryAcquireUploadPermit(long timeout, TimeUnit unit) {
        boolean acquired = globalUploadSemaphore.tryAcquire(timeout, unit);
        return acquired ? Optional.of(new UploadPermit(globalUploadSemaphore)) : Optional.empty();
    }
    
    /**
     * 获取当前可用许可证数量(用于监控和调度决策)
     */
    public int availablePermits() {
        return globalUploadSemaphore.availablePermits();
    }
}

// 许可证包装类(try-with-resources模式)
public class UploadPermit implements AutoCloseable {
    private final Semaphore semaphore;
    
    public UploadPermit(Semaphore semaphore) {
        this.semaphore = semaphore;
    }
    
    @Override
    public void close() {
        semaphore.release();  // 自动释放
    }
}
```
...
public void executeUpload(SubtaskContext subtask) {
    
    // 1. 尝试获取全局上传许可(最多等待30秒)
    try (UploadPermit permit = batchTransferManager.tryAcquireUploadPermit(30, TimeUnit.SECONDS)
            .orElseThrow(() -> new RuntimeException("获取上传许可超时, 当前并发度已满"))) {
    ...
    // 3. permit.close() 自动释放信号量(try-with-resources)
}
```
...
```java
// TaskConcurrencyLimiter.java (每个任务一个实例)
public class TaskConcurrencyLimiter {
    
    private final int taskMaxConcurrent;  // 该任务的并发上限
    private final Semaphore taskSemaphore;
    
    // 如果任务配置了max_concurrent_uploads, 使用任务专属值
    // 否则继承全局配置
    public TaskConcurrencyLimiter(int globalMax, Integer taskSpecificMax) {
        this.taskMaxConcurrent = (taskSpecificMax != null) 
            ? Math.min(taskSpecificMax, globalMax)  // 不超过全局上限
            : globalMax;
            
        this.taskSemaphore = new Semaphore(this.taskMaxConcurrent, true);
    }
    
    // 类似全局信号量的acquire/release方法...
}
```
...
```
场景: Agent同时运行3个任务(task_A, task_B, task_C)
全局并发上限: 3

task_A需要上传文件 → 申请许可 ✅ (剩余: 2)
task_B需要上传文件 → 申请许可 ✅ (剩余: 1)
task_C需要上传文件 → 申请许可 ✅ (剩余: 0)

task_D需要上传文件 → 申请许可 ❌ (阻塞等待)
    ↓
等待策略:
  1. 排队等待(FIFO) - 公平信号量保证
  2. 超时放弃(30秒) - 避免无限阻塞
  3. 降级处理 - 标记子任务RETRYING, 下次扫描再试
    
    ↓
当task_A的任一上传完成 → 释放许可 (剩余: 1)
task_D立即获得许可, 开始上传 ✅
```
...
```java
// PriorityUploadQueue.java
public class PriorityUploadQueue {
    ...
    private final PriorityBlockingQueue<UploadTask> queue = 
        new PriorityBlockingQueue<>(1000, 
            Comparator.comparingInt(t -> t.getPriority().value()));
    
    // 动态调整优先级的场景:
    // 1. 子任务处于RETRYING状态 → 提升为HIGH
    // 2. 文件大小 < 1MB → 保持NORMAL
    // 3. 文件大小 > 100MB → 降低为LOW(避免长时间占用许可证)
    // 4. 距离retry_max_days不足1天 → 提升为HIGH
}
```
...
```yaml
# agent.yml (Source Agent配置)
batch:
  transfer:
    # 全局并发控制
    max-concurrent-uploads: 3           # 全局最大并发上传数 [1, 10]
    upload-permit-timeout-seconds: 30   # 获取许可的超时时间
    permit-fairness: true              # 是否使用公平模式(防止饥饿)
    
    # 线程池配置
    thread-pool:
      core-size: 3                      # 核心线程数 (= max-concurrent-uploads)
      max-size: 6                       # 最大线程数 (= max-concurrent-uploads * 2)
      queue-capacity: 1000              # 工作队列容量(有界)
      rejection-policy: CALLER_RUNS     # 拒绝策略
      
    # 优先级调度
    priority-scheduling:
      enabled: true                     # 是否启用优先级队列
      retry-task-boost-factor: 2       # 重试任务的优先级提升倍数
      large-file-threshold-mb: 100      # 大文件阈值(超过则降低优先级)
      
    # 自适应调整(高级特性, Phase 3)
    auto-tuning:
      enabled: false                    # Phase 1默认关闭
      monitor-interval-seconds: 60     # 性能监控间隔
      target-cpu-usage: 70              # 目标CPU使用率(%)
      adjust-step: 1                    # 每次调整的步长(许可证数)
```
...
| 指标名称                             | 类型        | 说明           | 告警阈值           |
| -------------------------------- | --------- | ------------ | -------------- |
| `batch_upload_available_permits` | Gauge     | 当前可用上传许可证数   | = 0 持续>60s告警   |
| `batch_upload_queue_length`      | Gauge     | 等待上传的任务队列长度  | > 50 告警        |
| `batch_upload_wait_time_ms`      | Histogram | 获取许可证的等待时间分布 | P99 > 30s 告警   |
| `batch_task_active_uploads`      | Gauge     | 每个任务当前活跃上传数  | > 任务配置值告警      |
| `batch_thread_pool_active_count` | Gauge     | 上传线程池活跃线程数   | = max\_size 告警 |

**与现有AgentUploader的集成点**：

```
现有的 AgentUploader 类 (无需修改核心逻辑)
    ↓
新增 UploadListener 回调接口的实现类 BatchUploadListener
    ↓
BatchUploadListener 在 onChunkComplete() 时:
  ① 更新内存进度缓冲区
  ② 触发 ProgressReporter 异步上报
  
BatchUploadListener 在 onStart() 时:
  ① 调用 BatchTransferManager.acquireUploadPermit()
  ② 获得许可证后才开始上传
  
BatchUploadListener 在 onComplete()/onFailure() 时:
  ① 自动释放许可证 (try-with-resources)
  ② 更新最终状态到Proxy
```

* 单个Agent默认最大并发上传数：3（可通过配置调整）

* 单任务最大扫描文件数：10,000（防止OOM）

* 单文件最大大小：由Agent现有的`maxFileSize`配置控制
