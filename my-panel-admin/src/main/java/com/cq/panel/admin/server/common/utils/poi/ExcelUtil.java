...
import com.cq.panel.admin.server.common.utils.reflect.ReflectUtils;
...
public class ExcelUtil<T> {
    ...
    public String getAbsoluteFile(String filename) {
        String downloadPath = SpringUtils.getBean(AppConfig.class).getDownloadPath() + filename;
        File desc = new File(downloadPath);
        if (!desc.getParentFile().exists()) {
            final boolean result = desc.getParentFile().mkdirs();
            log.info("创建下载路径 {} 成功: {}", downloadPath, result);
        }
        return downloadPath;
    }
            ...
            if (!ArrayUtils.contains(this.excludeFields, field.getName())) {
                ...
                if (field.isAnnotationPresent(Excels.class)) {
                    ...
                    for (Excel attr : excels) {
                        if (!ArrayUtils.contains(this.excludeFields, field.getName() + "." + attr.targetAttr()) && (attr.type() == Excel.Type.ALL || attr.type() == type)) {
                            field.setAccessible(true);
                            fields.add(new Object[]{field, attr});
                        }
                    }
