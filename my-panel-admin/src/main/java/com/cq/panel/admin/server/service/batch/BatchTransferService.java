...
public class BatchTransferService
{
    ...
    public void pauseTask(Long taskId, String operatorId, String operatorName)
    {
        ...
        String oldStatus = task.getStatus();
        String sourceAgentApiUrl = buildAgentApiUrl(task.getSourceAgentId());
        proxyApiClient.pauseTask(taskId, sourceAgentApiUrl);
    ...
    public void resumeTask(Long taskId, String operatorId, String operatorName)
    {
        ...
        String sourceAgentApiUrl = buildAgentApiUrl(task.getSourceAgentId());
        proxyApiClient.resumeTask(taskId, sourceAgentApiUrl);
    ...
    public void cancelTask(Long taskId, String operatorId, String operatorName)
    {
        ...
        String oldStatus = task.getStatus();
        String sourceAgentApiUrl = buildAgentApiUrl(task.getSourceAgentId());
        proxyApiClient.cancelTask(taskId, sourceAgentApiUrl);
    ...
    public List<BatchTransferTask> listTasks(BatchTaskQueryDTO query)
    {
        ...
        if (query.getStatus() != null) entity.setStatus(query.getStatus());
        if (query.getSourceAgentId() != null) entity.setSourceAgentId(query.getSourceAgentId());
        if (query.getKeyword() != null) entity.setTaskName(query.getKeyword());
    ...
    private void validateCreateTask(BatchTaskCreateDTO dto)
    {
        ...
        {
            throw new IllegalArgumentException("目标目录数量(" + dirArr.length + ")必须与目标Agent数量(" + dto.getTargetAgents().size() + ")一致");
        }
        ...
        if ("BACKUP".equals(dto.getPostTransferAction()) && (dto.getBackupDir() == null || dto.getBackupDir().isEmpty()))
        {
    ...
    private void updateTaskFromProxyResult(Long taskId, Map<String, Object> result)
    {
        ...
        if (scanResultObj instanceof Map)
        {
            ...
            if (totalFiles instanceof Number) update.setTotalFiles(((Number) totalFiles).intValue());
            if (totalSizeBytes instanceof Number) update.setTotalSizeBytes(((Number) totalSizeBytes).longValue());
            update.setStatus("TRANSFERRING");
            batchTransferTaskService.update(update);
        }
    }

    private void logOperation(Long taskId, OperationType type, String oldVal, String newVal, String operatorId, String operatorName)
    {
    ...
    {
        return String.format("{\"scanFrequencySec\":%d,\"maxScanFiles\":%d,\"maxBandwidthKbS\":%s,\"retryEnabled\":%d,\"retryIntervalMin\":%d}",
