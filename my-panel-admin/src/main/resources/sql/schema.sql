...
-- ----------------------------
-- 8. 批量文件传输系统
-- 注: 设计文档中JSON字段使用JSON类型、状态字段使用ENUM类型，
--     为兼容H2数据库(MySQL兼容模式)，此处统一使用TEXT和VARCHAR替代。
-- ----------------------------
...
CREATE TABLE IF NOT EXISTS `batch_transfer_operation_log` (
    ...
    `operation_type` varchar(20) NOT NULL COMMENT '操作类型: CREATE-创建任务/START-启动任务/PAUSE-暂停任务/RESUME-恢复任务/CANCEL-取消任务/CONFIG_UPDATE-配置参数调整/MANUAL_RETRY-手动重试子任务/DELETE-删除任务, 设计类型:ENUM',
    `old_config` text DEFAULT NULL COMMENT '变更前的配置快照, 设计类型:JSON',
    `new_config` text DEFAULT NULL COMMENT '变更后的配置快照, 设计类型:JSON',
