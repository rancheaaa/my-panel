...
const AppManage = () => {
  ...
  const handleEdit = async (record) => {
    ...
    try {
        const res = await getProject(record.id);
        if (res.code === 200) {
  ...
  const handleModalOk = async () => {
    try {
      ...
      if (currentId) {
        await updateProject({ ...values, id: currentId });
  ...
  return (
    <div className="app-manage-page-container">
      <Card bordered={false} className="search-card" style={{ marginBottom: 16, flexShrink: 0 }}>
        <Form form={form} layout="inline" component="div" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} style={{ width: '100%' }}>
          <Row gutter={[24, 16]} style={{ width: '100%' }}>
            <Col span={6}>
              <Form.Item name="projectName" label="应用名称">
      ...
      <Card bordered={false} className="table-card" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
        ...
        <div className="app-manage-table-container">
          <Table
            rowSelection={{
              selectedRowKeys,
